    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-40 hidden md:block">
          ...
          <div className="mb-3 w-64 rounded-xl border border-ink/10 bg-surface p-2 shadow-xl">
            {PANEL_OPTIONS.map((option) => (
              <Link
                ...
                className="block rounded-lg px-4 py-3 text-sm font-medium text-ink transition hover:bg-carbon"
              >
                {option.label}
              </Link>
            ))}
        ...
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="rounded-full bg-gold px-5 py-3 text-xs font-semibold uppercase tracking-wide text-paper shadow-lg transition hover:opacity-90"
        >
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-ink/10 bg-paper/95 p-3 backdrop-blur md:hidden">
        <Link
          href="/contacto"
          className="block w-full rounded-lg bg-gold py-3 text-center text-sm font-semibold uppercase tracking-wide text-paper"
        >
          HABLEMOS →
        </Link>
      </div>
