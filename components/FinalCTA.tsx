      ...
      <div className="mx-auto max-w-5xl text-center">
        <Reveal>
          <h2 className="text-3xl font-bold uppercase leading-tight tracking-tight sm:text-5xl">
            ...
            <br />
            <span className="text-deep">Las buenas implementaciones</span>
            <br />
            <span className="text-deep">cambian negocios.</span>
          </h2>
        ...
        <div className="mt-14 grid gap-6 text-left sm:grid-cols-3">
          {PATHS.map((path, i) => (
            <Reveal key={path.title} delay={i * 0.1} className="h-full">
              <div className="h-full rounded-2xl border border-paper/15 p-8 transition duration-300 hover:-translate-y-1.5 hover:border-accent/40 hover:bg-paper/5">
                ...
                <div className="mt-6">
                  <ServiceCta
                    ...
                    label={path.label}
                    placement="final_cta"
                    className="inline-flex items-center gap-2 rounded-full bg-paper px-5 py-3 text-xs font-semibold uppercase tracking-wide text-ink transition hover:bg-gold hover:text-paper"
                  />
                </div>
              </div>
            </Reveal>
        ...
        <Reveal className="mt-16 border-t border-paper/15 pt-10" delay={0.15}>
          ...
          <a
            href="/contacto"
            className="mt-4 inline-block text-sm font-semibold uppercase tracking-wide text-deep underline-offset-4 hover:underline"
          >
            CUÉNTAMELA →
          </a>
        </Reveal>
